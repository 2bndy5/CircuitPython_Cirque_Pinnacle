...
MUX_PNP = const(4)
MUX_NPN = const(1)
CRTL_REPEAT = const(128)
CRTL_PWR_IDLE = const(64)
...
class PinnacleTouch:
    ...
    def __init__(self, dr_pin=None):
        ...
        self._rap_write(10, 30)  # z-idle packet count
    ...
    def data_mode(self, mode):
        ...
        sys_config = self._rap_read(3) & 231  # clear AnyMeas mode flags
        if mode in (RELATIVE, ABSOLUTE):
            if self.data_mode == ANYMEAS:  # if leaving AnyMeas mode
                ...
                self._rap_write(7, 30)  # enables all compensations
                self._rap_write(10, 30)  # 30 z-idle packets
            ...
            self._rap_write(3, sys_config | 8)
    ...
    def read(self):
        ...
        if self.data_mode == ABSOLUTE:  # if absolute mode
            ...
            return_vals[0] &= 63  # buttons
            return_vals[2] |= (return_vals[4] & 15) << 8  # x
            return_vals[3] |= (return_vals[4] & 240) << 4  # y
            return_vals[5] &= 63  # z
    ...
    def allow_sleep(self, is_enabled):
        self._rap_write(3, (self._rap_read(3) & 251) | (is_enabled << 2))
    ...
    def shutdown(self, is_off):
        self._rap_write(3, (self._rap_read(3) & 253) | (is_off << 1))
    ...
    def sample_rate(self, val):
        if self.data_mode != ANYMEAS:
            if val in (200, 300):
                ...
                self._era_write_bytes(414, reload_timer, 2)
                val = 0
            else:
                # enable palm & noise compensations
                self._rap_write(6, 0)
                self._era_write_bytes(414, 19, 2)
    ...
    def detect_finger_stylus(self, enable_finger=True,
        ...
        finger_stylus = self._era_read(235)
        finger_stylus |= (enable_stylus << 2) | enable_finger
        self._era_write(235, finger_stylus)
    ...
    def calibration_matrix(self):
        ...
        # 16-bits integers
        return list(struct.unpack('46h', self._era_read_bytes(479, 92)))
    ...
    def calibration_matrix(self, matrix):
        ...
        for index in range(46):
            ...
            self._era_write(479 + index * 2, buf[0])
            self._era_write(479 + index * 2 + 1, buf[1])
    ...
    def set_adc_gain(self, sensitivity):
        ...
        val = self._era_read(391) & 63 | (sensitivity << 6)
        self._era_write(391, val)

    def tune_edge_sensitivity(self, x_axis_wide_z_min=4,
                              y_axis_wide_z_min=3):
        """Changes thresholds to improve detection of fingers."""
        self._era_write(329, x_axis_wide_z_min)
        self._era_write(360, y_axis_wide_z_min)
    ...
    def start_measure_adc(self, bits_to_toggle, toggle_polarity):
        ...
        if self._mode == ANYMEAS:
            ...
            for i in range(3, -1, -1):
                tog_pol.append((bits_to_toggle >> (i * 8)) & 255)
            for i in range(3, -1, -1):
                tog_pol.append((toggle_polarity >> (i * 8)) & 255)
            # write toggle and polarity parameters to register 0x13 - 0x1A
            self._rap_write_bytes(19, tog_pol)
            # initiate measurements
            self._rap_write(3, self._rap_read(3) | 30)
    ...
    def get_measure_adc(self):
        ...
        if not self.dr_pin.value:
            return None
        result = self._rap_read_bytes(17, 2)
        self.clear_status_flags()
        return result
    ...
    def _era_read(self, reg):
        ...
        self._rap_write(30, 1)  # indicate reading only 1 byte
        while self._rap_read(30):  # read until reg == 0
            pass  # also sets Command Complete flag in Status register
        buf = self._rap_read(27)  # get value
        self.clear_status_flags()
    ...
    def _era_read_bytes(self, reg, numb_bytes):
        ...
        for _ in range(numb_bytes):
            self._rap_write(30, 5)  # indicate reading sequential bytes
            while self._rap_read(30):  # read until reg == 0
                pass  # also sets Command Complete flag in Status register
            buf += bytes([self._rap_read(27)])  # get value
            self.clear_status_flags()
        self.feed_enable = prev_feed_state  # resume previous feed state
        return buf
    ...
    def _era_write(self, reg, value):
        prev_feed_state = self.feed_enable
        self.feed_enable = False  # accessing raw memory, so do this
        self._rap_write(27, value)  # write value
        self._rap_write_bytes(28, [reg >> 8, reg & 255])
        self._rap_write(30, 2)  # indicate writing only 1 byte
        while self._rap_read(30):  # read until reg == 0
            pass  # also sets Command Complete flag in Status register
        self.clear_status_flags()
        self.feed_enable = prev_feed_state  # resume previous feed state
    ...
    def _era_write_bytes(self, reg, value, numb_bytes):
        ...
        self._rap_write(27, value)  # write value
        self._rap_write_bytes(28, [reg >> 8, reg & 255])
        self._rap_write(30, 10)  # indicate writing sequential bytes
        for _ in range(numb_bytes):
            while self._rap_read(30):  # read until reg == 0
                pass  # also sets Command Complete flag in Status register
            self.clear_status_flags()
...
class PinnacleTouchI2C(PinnacleTouch):
    ...
    def _rap_read_bytes(self, reg, numb_bytes):
        buf = bytes([reg | 160])  # per datasheet
    ...
    def _rap_write_bytes(self, reg, values):
        ...
        for index, byte in enumerate(values):
            ...
            buf += bytes([(reg + index) | 128, byte & 63])
        with self._i2c as i2c:
            i2c.write(buf)
